...
class MedicalImageFusion:
    ...
    def is_valid_medical_image(self, image_path, img_type="GENERIC"):
        ...
        try:
            ...
            # 2. Black Background Check (Medical scans almost always have a noticeable amount of pitch-black pixels)
            img_gray = np.mean(arr, axis=2)
            dark_ratio = float(np.sum(img_gray < 25)) / img_gray.size
            
            # Documents, whiteboards, and most daylight photos have very few pure black pixels.
            if dark_ratio < 0.05:
                return False
                
            # 3. Corner Texture and Brightness
            h, w = arr.shape[:2]
            corners_noisy = False
            corners_bright = False
            ...
            if h > 20 and w > 20:
                ...
                # Medical scans have black borders, if corners are bright (>60), it's likely a photo or document
                if sum(mean > 60 for mean in corner_means) >= 2:
                    corners_bright = True

            # If corners are bright or there is almost no black background, it's not a medical scan.
            if corners_bright:
                return False
                
            # 4. Content Volume & Flat Color Analysis (Catches Terminal Screenshots & UI)
            # "Tissue" pixels are anything not deeply dark (so actual brains, or text in a terminal)
            tissue_pixels = img_gray[img_gray >= 30]
            if tissue_pixels.size > 0:
                tissue_ratio = float(tissue_pixels.size) / img_gray.size
                
                # A brain/body occupies a decent chunk of the image. 
                # Terminals with a few lines of text are mostly empty black space.
                if tissue_ratio < 0.05:
                    return False
                    
                # Artificial screenshots (UI, Terminals, Diagrams) have huge patches of exactly uniform colors.
                # Natural MRI scans have smooth gradients and Rician noise.
                hist, _ = np.histogram(tissue_pixels, bins=225, range=(30, 255))
                max_bin_ratio = float(np.max(hist)) / tissue_pixels.size
                
                # If a single exact pixel intensity accounts for > 40% of all "tissue", it's artificial graphic/text.
                if max_bin_ratio > 0.40:
                    return False

            gy = float(np.abs(np.diff(img_gray, axis=0)).mean())
            gx = float(np.abs(np.diff(img_gray, axis=1)).mean())
            edge_density = (gx + gy) / 2.0

            if img_type.upper() == "FUSED":
                return True
            elif img_type.upper() == "MRI":
                if avg_color_variance > 2.0:
                    return False  # Reject colored images (like PET) for MRI
                if edge_density < 3.0:
                    return False  # Reject blurry images (like grayscale PET) for MRI
            elif img_type.upper() == "PET":
                if avg_color_variance < 2.0 and edge_density > 3.0:
                    return False  # Reject grayscale AND sharp images (like MRI) for PET
    ...
    def _get_real_gemini_report(self, image_path, img_type):
        ...
        try:
            ...
            if img_type.upper() == 'MRI':
                ...
                CRITICAL FIRST STEP: Verify if the uploaded image is genuinely a valid MRI medical scan of a human brain or body. If the image is NOT a medical scan (e.g., it is a photo of an animal, car, face, general object, or drawing), you MUST return EXACTLY this JSON and nothing else:
                {{ "error": true, "title": "IMAGE VALIDATION FAILED", "impression": "The uploaded image is not a valid MRI scan.", "findings": {{ "general": ["Image is not a medical scan."] }} }}

                If it IS a valid MRI scan, deeply analyze this MRI medical scan and determine exactly what problem the patient is facing based exclusively on the visual evidence.
                ...
                [Evaluation ID: {request_id}]
                Act as an expert Radiologist. 
                CRITICAL FIRST STEP: Verify if the uploaded image is genuinely a valid PET medical scan of a human brain or body. If the image is NOT a medical scan (e.g., it is a photo of an animal, car, face, general object, or drawing), you MUST return EXACTLY this JSON and nothing else:
                {{ "error": true, "title": "IMAGE VALIDATION FAILED", "impression": "The uploaded image is not a valid PET scan.", "findings": {{ "general": ["Image is not a medical scan."] }} }}

                If it IS a valid PET scan, deeply analyze this PET medical scan and determine exactly what problem the patient is facing based exclusively on the visual evidence.
                ...
                [Evaluation ID: {request_id}]
                Act as an expert Radiologist. 
                CRITICAL FIRST STEP: Verify if the uploaded image is genuinely a valid {img_type} medical scan of a human brain or body. If the image is NOT a medical scan (e.g., it is a photo of an animal, car, face, general object, or drawing), you MUST return EXACTLY this JSON and nothing else:
                {{ "error": true, "title": "IMAGE VALIDATION FAILED", "impression": "The uploaded image is not a valid {img_type} scan.", "findings": {{ "general": ["Image is not a medical scan."] }} }}

                If it IS a valid {img_type} scan, deeply analyze this {img_type} medical scan and determine exactly what problem the patient is facing based exclusively on the visual evidence.
